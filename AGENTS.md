# USBRadioPlus workflow development rules
...
packages manually only when explicitly requested, without Debian 12 tests or
staged-install checks. Automated releases publish Debian 13 packages only;
node installations use Debian 13 arm64 packages.
...
Keep reusable workflow implementations in this repository and production-code
callers thin. Validate workflow repairs independently of production validation,
so a broken workflow can be repaired without requiring the production gate it
implements to pass first. Validate workflow edits with Actionlint.
